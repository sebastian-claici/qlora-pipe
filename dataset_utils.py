        ...
        while len(tokens) > 0:
            ...
            if len(example_tokens) >= sequence_len:
                ...
                if (
                    ...
                    and example_tokens[0] != tokenizer.bos_token_id
                ):
...
def load_raw_dataset(
    ...
    eval_size,
    overlap=0,
    subsample_documents=None,
):
    if dataset_path.endswith(".txt"):
        dataset = datasets.load_dataset(
            "text", data_files=dataset_path, sample_by="document"
        )["train"]
    ...
    if subsample_documents:
        dataset = dataset.shuffle(seed=13).select(
            list(range(int(subsample_documents * len(dataset))))
        )
    ...
    dataset = dataset.map(
        lambda x: {
            "input_ids": list(
                yield_sequences_from_token_batch(
                    tokenizer, x["input_ids"], sequence_len
                )
            )
        },
        batched=True,
    ...
    if eval_size > 0:
        split_datasets = dataset.train_test_split(
            test_size=eval_size, shuffle=True, seed=42
        )
...
def load_single_dataset(
    dataset_path, dataset_type, tokenizer, sequence_len, eval_size, subsample=None
):
    if dataset_type in ["textfile", "doclist"]:
        with zero_first(is_main_process()):
            train_data, eval_data = load_raw_dataset(
                dataset_path, tokenizer, sequence_len, eval_size
            )
    elif dataset_type == "axolotl":
        train_data, eval_data = load_axolotl_dataset(
            dataset_path, tokenizer, sequence_len, eval_size
        )
    else:
        raise NotImplementedError()
    ...
    if mode == "concatenate":
        ...
        if "batch_size_tokens" in config:
            ...
            avg_lengths = torch.tensor(
                [
                    dataset["length"].to(torch.float32).mean()
        ...
        dataset = datasets.interleave_datasets(
            ...
            probabilities=probs,
            seed=42,
            stopping_strategy=config.get(
                "dataset_interleave_stopping_strategy", "first_exhausted"
            ),
        )
    else:
    ...
    if len(train_datasets) == 1:
        ...
        with zero_first(is_main_process()):
            train_dataset = combine_datasets(
                train_datasets, config, sample_weights=sample_weights
            )
