...
class Saver:
    def __init__(
        ...
        self.best_loss = None
        best_loss_path = self.save_root / "best_loss.txt"
        if os.path.exists(best_loss_path) and os.path.isdir(
            self.save_root / "best_loss"
        ):
            ...
            for name, p in self.pipeline_model.named_parameters():
                if p.requires_grad:
                    if not hasattr(p, "original_name"):
                        ...
                        continue
                    partial_state_dict[
                        p.original_name.replace(".default", "").replace(
                            ".modules_to_save", ""
                        )
                    ] = p.detach()
                    if "save_dtype" in self.config:
                        convert_state_dict_dtype(
                            partial_state_dict, self.config["save_dtype"]
                        )
            torch.save(
                partial_state_dict, os.path.join(tmp_dir, f"state_dict_{stage_id}.bin")
            )
    ...
    def save_full_model(self, name, max_shard_size="5GB"):
        ...
        if dp_id == 0:
            ...
            partial_state_dict = {
                p.original_name: p.detach() for p in self.pipeline_model.parameters()
            }
            if "save_dtype" in self.config:
    ...
    def will_save(self, type, name):
        ...
        if type == "step":
            ...
            if len(self.chrono_states["step"]) > self.keep_states:
                ...
                shutil.rmtree(
                    self.save_root / self.chrono_states["step"].pop(0),
                    ignore_errors=True,
                )
            ...
            if len(self.chrono_states["global_step"]) > self.keep_states:
                ...
                shutil.rmtree(
                    self.save_root / self.chrono_states["global_step"].pop(0),
                    ignore_errors=True,
                )
        else:
    ...
    def process_step(self, step):
        ...
        pending_save_best_loss = os.path.exists(
            self.save_root / ".pending_save_best_loss"
        )
        if pending_save_best_loss:
            ...
            if is_main_process():
                if self.old_best is not None:
                    print(
                        f"New best loss: {self.best_loss} from {self.old_best} (Δ{self.old_best - self.best_loss} = {100 * (self.old_best - self.best_loss) / self.old_best:.2f}%)"
                    )
                else:
                    print(f"New best loss: {self.best_loss}")
                os.replace(
                    self.save_root / ".pending_save_best_loss",
                    self.save_root / "best_loss.txt",
                )
    ...
    def append_eval_results(self, loss, save_best=True):
        if loss is not None:
            if self.best_loss is None:
                ...
                print(
                    f"Evaluation loss: {loss} (best: {self.best_loss}, Δ: {self.best_loss - loss} = {100 * (self.best_loss - loss) / self.best_loss:.2f}%)"
                )
            if self.best_loss is None or loss < self.best_loss:
                ...
                if save_best:
                    with open(
                        os.path.join(self.save_root, ".pending_save_best_loss"), "w"
                    ) as f:
