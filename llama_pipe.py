    ...
    def forward(self, inputs):
        ...
        if self.model[0].config.model_type == "gemma2":
            normalizer = torch.tensor(
                self.model[0].config.hidden_size ** 0.5, dtype=hidden_states.dtype
            )
...
class LlamaDecoderLayerPipe(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.mlp_offloaded_to_cpu:
            ...
            cpu_up_proj = move_data_to_device(
                self.orig.mlp.up_proj, hidden_states.device
            )
            cpu_down_proj = move_data_to_device(
                self.orig.mlp.down_proj, hidden_states.device
            )
            cpu_gate_proj = move_data_to_device(
                self.orig.mlp.gate_proj, hidden_states.device
            )
        result = (
            self.orig(
...
class LlamaForCausalLMPipe(PipelineModel, transformers.LlamaForCausalLM):
    ...
    def to_layer_specs(self):
        ...
        result.append(
            LayerSpec(
                LlamaRMSNormPipe, self.loader_util, self.model.norm, _estimated_size=0
            )
        )
        result.append(
