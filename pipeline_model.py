...
def top_k_accuracy(logits, labels, k_list, ignore_index=-100):
    ...
    for k in k_list:
        accuracies.append(
            torch.any(top_k_predictions[:, :k] == labels, dim=-1)
            .to(torch.float32)
            .mean()
        )
    return accuracies
...
class ComputeMetrics(nn.Module):
    ...
    def forward(self, inputs):
        ...
        extra_ignored_labels = torch.full(
            (labels.shape[0], 1), -100, device=logits.device
        )
        ...
        if self.focal_loss_gamma > 0:
            ...
            optimized_loss_unreduced = (
                1 - p
            ) ** self.focal_loss_gamma * optimized_loss_unreduced
    ...
    def __init__(self, config, quantization_config, model_config):
        if config["full_fine_tune"] and model_config.tie_word_embeddings:
            raise NotImplementedError(
                "FFT is not supported for models with tied embeddings"
            )
        self.train_config = config
        self.modules_to_not_quantize = get_keys_to_not_convert(self)
        self.loader_util = LoaderUtil(
            config["model"], quantization_config, self.modules_to_not_quantize
        )
...
def _replace_with_quantized_linear(
...
def _replace_with_bnb_linear(parent_modules_map, name, full_name, quantization_config):
    ...
    if (
        ...
        and _partial_module_name_match(
    ...
    for name, module in model.named_children():
        ...
        if (
            ...
            if not any(
                ...
                _replace_with_quantized_linear(
                    model._modules, name, current_key_name_str, quantization_config
                )
...
class LoaderUtil:
    def __init__(self, model_path, quantization_config, modules_to_not_quantize):
        ...
        index_file = os.path.join(
            model_path, transformers.utils.SAFE_WEIGHTS_INDEX_NAME
        )
    ...
    def get_partial_state_dict(self, leaf_file):
        if self.loaded_state_dict is None or leaf_file != self.loaded_state_dict[0]:
            ...
            state_dict = transformers.modeling_utils.load_state_dict(
                os.path.join(self.model_path, leaf_file)
            )
    ...
    def load_state_dict_into_module(self, module):
        ...
        param_renaming_map = {
            p.original_name: new_name for new_name, p in module.named_parameters()
        }
        ...
        if self.checkpoint_metadata is not None:
            ...
            needed_checkpoint_files = set(
                weight_map[key.replace("orig.", "")] for key in expected_keys
            )
        else:
        ...
        for checkpoint_file in needed_checkpoint_files:
            ...
            renamed_state_dict = {
                param_renaming_map[k]: v
                for k, v in state_dict.items()
                if k in param_renaming_map
            }
