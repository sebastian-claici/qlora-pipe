...
parser.add_argument(
    "--debug_dataset",
    type=int,
    help="print out this many training examples and then quit",
)
parser.add_argument(
...
def apply_max_norm_regularization(model, config):
    ...
    if len(norms) > 0:
        ...
        if torch.any(torch.isnan(norms)):
            raise RuntimeError(
                f"NaN detected in norms, probably some/all weights are NaN"
            )
...
def load_pipeline_model_with_lora(config, model_type):
    ...
    if config.get("quantization", None):
        ...
        if bnb_quant_config := config["quantization"].get("bnb", None):
            if bnb_compute_dtype := bnb_quant_config.get(
                "bnb_4bit_compute_dtype", None
            ):
                bnb_quant_config["bnb_4bit_compute_dtype"] = DTYPE_MAP[
                    bnb_compute_dtype
                ]
            ...
            if llm_int8_skip_modules := bnb_quant_config.get(
    ...
    if model_type == "llama":
        model = llama_pipe.LlamaForCausalLMPipe(
        ...
        model = llama_pipe.Qwen2ForCausalLMPipe(
            config, quantization_config=quantization_config
        )
    elif model_type == "mistral":
        model = llama_pipe.MistralForCausalLMPipe(
            config, quantization_config=quantization_config
        )
    else:
    ...
    for layer in layers:
        if (
            isinstance(layer, LayerSpec)
            and "decoderlayer" in layer.typename.__name__.lower()
        ):
    ...
    target_modules = (
        config["target_modules"] if "target_modules" in config else "all-linear"
    )
    if full_fine_tune:
        lora_model = None
        lora_config = None
        ...
        if isinstance(target_modules, list):
            for name, p in model.named_parameters():
                if not any(target in name for target in config["target_modules"]):
                    ...
                    print(
                        f"not training {name} because it is not present in target_modules"
                    )
    else:
        layers_to_transform = (
            parse_layers_to_transform(config["layers_to_transform"])
            if "layers_to_transform" in config
            else None
        )
...
if __name__ == "__main__":
    ...
    # Ugly hack so we can move quantized models from GPU to CPU, 
    # and back to GPU again without triggering quantization a second time.
    ...
    pipeline_model, lora_model, lora_config = load_pipeline_model_with_lora(
        config, model_type
    )
    ...
    if config.get("offload_mlp_to_cpu", False):
        ...
        ]  # MLP offloading only works with activation checkpointing
    ...
    train_dataloader = dataloader.PipelineDataLoader(
        ...
        if "group_by_length" not in config
        else config["group_by_length"],
        batch_size_tokens=None
        if "batch_size_tokens" not in config
        else config["batch_size_tokens"],
    )
    ...
    if is_main_process():
        ...
        fraction_evaling = relative_eval_time / (
            relative_eval_time + relative_train_time
        )
        print()
        print(
        ...
        if fraction_evaling > 0.15:
            print(
                f"WARNING: eval dataset is unusually large compared to eval_steps. We will spend a lot of time evaluating. Lowering eval_size and/or bumping eval_steps is recommended."
            )
        print()
    ...
    if (
        "lr_scheduler" not in config
        or config["lr_scheduler"] == "constant"
        or config["lr_scheduler"] == "none"
    ):
    ...
    eval_gradient_accumulation_steps = (
        ...
        if "eval_gradient_accumulation_steps" in config
        else 1
    )
    ...
    if (
        "eval_before_first_step" in config
        and config["eval_before_first_step"]
    ...
    while True:
        ...
        if lora_config is not None:
            keys_scaled, avg_norm, max_norm, norms = apply_max_norm_regularization(
