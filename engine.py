...
def initialize(args=None, model=None, model_parameters=None, optimizer=None):
        ...
        if self.is_last_stage():
            ...
            for outputs in grouped_outputs:
                ...
                if self.is_data_parallel:
                    ...
                    gather_result = [
                        torch.zeros(size).to(self.device) for size in sizes
                    ]
                    dist.all_gather(
                        gather_result,
                        concat_outputs,
            ...
            result = [None]
            torch.distributed.broadcast_object_list(
                result, src=src_rank, group=self.grid.get_pipe_parallel_group()
            )
            agg_sizes = result[0]
...
class CustomPipelineModule(PipelineModule):
    ...
    def _partition_layers(self, method="uniform"):
        ...
        if method == "uniform":
            ...
            estimated_sizes = [
                getattr(l, "estimated_size", 0) for l in self._layer_specs
            ]
            self.parts = ds_utils.partition_balanced(
                weights=estimated_sizes, num_parts=num_stages
            )
        else:
            raise NotImplementedError(f"Partitioning method {method} not implemented.")
